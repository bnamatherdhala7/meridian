...
def run_spl_query_tool(spl: str, index: str = "*", time_window: str = "last 30 minutes") -> dict:
    """Execute an SPL query against Splunk indexes. Returns matching events and statistics."""
...
def search_indexes_tool(query: str = "") -> dict:
    """Discover available Splunk indexes and their metadata."""
    return search_indexes(query)


@mcp.tool()
def get_knowledge_objects_tool(object_type: str = "all") -> dict:
    """Surface saved searches, field extractions, and lookups from Splunk."""
    return get_knowledge_objects(object_type)


@mcp.tool()
def get_network_topology_tool(site: str = "", device_id: str = "") -> dict:
    """Get Cisco Catalyst network topology — device graph and inter-device links."""
...
def get_telemetry_metrics_tool(
    ...
    """Get real-time interface-level telemetry metrics from a Cisco device."""
